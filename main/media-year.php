    ...
    <section class="categories-grid-section spad">
        <div class="container" id="anime-movies">
            <div class="row">
                <div class="col-lg-12">
                    <div class="title">
                        <a href="media-year.php?year=<?= $year; ?>"> All Media of Year (<?= $year; ?>) (<?= $dbData->get_media_year_num($year); ?>)</a>
                    </div>
                ...
                <div class="col-lg-12 p-0">
                    <div class="row">
                        ...
                        $limit = 12;
                        $total_rows = $dbData->get_media_year_num($year);
                        ...
                        if (isset($_GET['order'])) {
                            if ($_GET['order'] == "alphaAsc") {
                                $rows = $dbData->get_media_year($year, "title", "ASC", $initial_page, $limit);
                            } else if ($_GET['order'] == "alphaDesc") {
                                $rows = $dbData->get_media_year($year, "title", "DESC", $initial_page, $limit);
                            } else if ($_GET['order'] == "idAsc") {
                                $rows = $dbData->get_media_year($year, "id", "ASC", $initial_page, $limit);
                            } else if ($_GET['order'] == "ratingAsc") {
                                $rows = $dbData->get_media_year($year, "id", "DESC", $initial_page, $limit);
                            }
                        } else {
                            $rows = $dbData->get_media_year($year, "released_dt", "DESC", $initial_page, $limit);
                        }
                        foreach ($rows as $row) :
                    ...
                    <div class="pagination-item text-center">
                        ...
                        if ($page_number >= 2) {
                            $href = "media-year.php?page=$start&year=$year";
                        ?>
                            <a href="media-year.php?page=1&year=<?= $year ?>">
                        ...
                        for ($i = $start; $i <= $end; $i++) :
                            ...
                            //-- Set href
                            $href = "media-year.php?page=$i&year=$year";
                        ...
                        ?>
                            ...
                            <a href="media-year.php?page=<?= $total_pages ?>&year=<?= $year ?>">
