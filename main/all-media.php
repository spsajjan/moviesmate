            ...
            <div class="row">
                ...
                <div class="col-lg-12 p-0">
                    <div class="row">
                        ...
                        $limit = 12;
                        $total_rows = $dbData->get_media_num();
                        // get the required number of pages
                        $total_pages = ceil($total_rows / $limit);
                        //-- If page number exists the get page number else set it to 1
                        if (!isset($_GET['page'])) {
                            $page_number = 1;
                        } else {
                        ...
                        if (isset($_GET['order'])) {
                            if ($_GET['order'] == "alphaAsc") {
                                $rows = $dbData->get_spec_media("title", "ASC", $initial_page, $limit);
                            } else if ($_GET['order'] == "alphaDesc") {
                                $rows = $dbData->get_spec_media("title", "DESC", $initial_page, $limit);
                            } else if ($_GET['order'] == "idAsc") {
                                $rows = $dbData->get_spec_media("id", "ASC", $initial_page, $limit);
                            } else if ($_GET['order'] == "ratingAsc") {
                                $rows = $dbData->get_spec_media("rating", "DESC", $initial_page, $limit);
                            }
                        } else {
                            $rows = $dbData->get_spec_media("released_dt", "DESC", $initial_page, $limit);
                        }
                        foreach ($rows as $row) :
                    ...
                    <div class="pagination-item text-center">
                        ...
                        if ($page_number >= 2) {
                            $href = "all-media.php?page=$start";
                        ?>
                            <a href="all-media.php?page=1">
                        ...
                        ?>
                            ...
                            <a href="all-media.php?page=<?= $total_pages ?>">
